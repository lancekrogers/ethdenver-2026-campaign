# Festival Rules: Bounty Completion
...
### Go Agent Rules (agent-coordinator, agent-defi, agent-inference)

- Always pass `context.Context` as the first parameter for any I/O operation
- Always check `ctx.Err()` before starting long operations
- Never ignore context cancellation
- Use the project's established error framework — never use `fmt.Errorf` in production code
- Always wrap errors with contextual information about the failed operation
- All Go binaries MUST be output to the `bin/` directory (e.g., `go build -o bin/ ./cmd/...`)
- Never place compiled binaries in the project root or other directories
- `.gitignore` must use `bin/` directory pattern, never bare binary names
- Use `just` for all build, test, lint, and run commands — never invoke `go build` or `go test` directly

### Solidity Contract Rules (projects/contracts)

- Follow Solidity best practices: checks-effects-interactions pattern, explicit visibility, named return values
- Use OpenZeppelin libraries (Ownable, etc.) via forge install, not copy-paste
- All contracts must compile with `forge build` before any tests are written
- Forge tests go in `test/` directory and must use the Test suffix (e.g., `AgentSettlementTest`)
- Deploy scripts go in `script/` directory and must extend `Script`
...
- `just build` must pass for all Go projects before any task is marked complete
- `forge build` must pass for contracts before any task is marked complete
- `just test` must pass with no failures before a sequence is marked complete
- `forge test` must pass with no failures before the contracts sequence is marked complete
- Docker images must build successfully before the festival is marked complete
