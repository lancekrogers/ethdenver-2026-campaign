...
```bash
cd projects/agent-inference && just test
```
...
```bash
cd projects/agent-inference && just test -run Integration
```
...
```bash
cd projects/agent-inference && go test -coverprofile=coverage.out ./internal/zerog/... && go tool cover -func=coverage.out
```
