---
fest_type: gate
fest_id: 06_review.md
fest_name: Code Review
fest_parent: 02_analysis_pipeline
fest_order: 6
fest_status: pending
fest_autonomy: low
fest_gate_type: review
fest_created: 2026-03-13T02:27:19.942006-06:00
fest_tracking: true
---

# Task: Code Review

**Task Number:** 6 | **Parallel Group:** None | **Dependencies:** Testing and Verification | **Autonomy:** low

## Objective

Review all code changes in this sequence for quality, correctness, and adherence to project standards.

## Review Checklist

### Code Quality

- [ ] Code is readable and well-organized
- [ ] Functions/methods are focused (single responsibility)
- [ ] No unnecessary complexity
- [ ] Naming is clear and consistent
- [ ] Comments explain "why" not "what"

### Architecture & Design

- [ ] Changes align with project architecture
- [ ] No unnecessary coupling introduced
- [ ] Dependencies are appropriate
- [ ] Interfaces are clean and focused
- [ ] No code duplication

### Standards Compliance

```bash
golangci-lint run ./internal/analysis/...
```

- [ ] Linting passes without warnings
- [ ] Formatting is consistent
- [ ] Project conventions are followed

### Sequence-Specific Review Focus

**Files/packages to review:**
- `internal/analysis/claude/` - Claude API client and prompt templates
- `internal/analysis/normalizer.go` - Market data normalization
- `internal/analysis/resolver.go` - Resolution rule analysis prompt chain
- `internal/analysis/signal.go` - Signal generation from LLM output
- `internal/analysis/types.go` - NormalizedMarket, Signal structs

**Design patterns to verify:**
- [ ] LLM prompts are stored as templates, not inline strings
- [ ] Claude client is injected via interface (testable with mock LLM)
- [ ] Signal struct includes reasoning text traceable to input data
- [ ] Resolution analysis handles edge cases where title and rules diverge
- [ ] Timeout configuration for Claude API calls (30s max)
- [ ] No raw LLM output leaks into domain types without parsing/validation

### Error Handling

- [ ] Errors are handled appropriately
- [ ] Error messages are helpful
- [ ] No panic/crash scenarios
- [ ] Resources are properly cleaned up

### Security Considerations

- [ ] No secrets in code
- [ ] Claude API key loaded from environment
- [ ] No PII in prompts or logs
- [ ] LLM responses sanitized before use

### Performance

- [ ] No obvious performance issues
- [ ] Claude API calls are async-capable
- [ ] No memory leaks
- [ ] Analysis results cacheable for repeated queries

### Testing

- [ ] Tests are meaningful
- [ ] Edge cases covered
- [ ] Test data is appropriate
- [ ] Mocks used correctly

## Review Process

1. **Read the sequence goal** - Understand what was being built
2. **Review file by file** - Check each modified file
3. **Run the code** - Verify functionality works
4. **Document findings** - Note issues and suggestions

## Findings

### Critical Issues (Must Fix)

1. [ ] [Issue description and recommendation]

### Suggestions (Should Consider)

1. [ ] [Suggestion and rationale]

### Positive Observations

- [Note good patterns or practices observed]

## Definition of Done

- [ ] All files reviewed
- [ ] Linting passes
- [ ] No critical issues remaining
- [ ] Suggestions documented
- [ ] Knowledge shared with team (if applicable)

## Review Summary

**Reviewer:** [Name/Agent]
**Date:** [Date]
**Verdict:** [ ] Approved / [ ] Needs Changes

**Notes:**
[Summary of the review and any outstanding concerns]
