...
```bash
cd projects/agent-defi && go test -tags=integration ./...
```
...
- Minimum coverage: N/A (verification sequence, no new code) for new code

```bash
cd projects/agent-defi && go test -cover ./...
```
