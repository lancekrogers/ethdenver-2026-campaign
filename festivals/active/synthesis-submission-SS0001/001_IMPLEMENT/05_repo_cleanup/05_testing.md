...
```bash
# Scan for remaining secrets after cleanup
grep -r "PRIVATE_KEY\|sk-\|api_key" --include="*.env*" --include="*.json" . || echo "No secrets found"
```
...
git clone --recurse-submodules https://github.com/obedience-corp/obey-agent-economy.git /tmp/test-clone
```
...
- Minimum coverage: N/A (repo cleanup, no new code) for new code

N/A for repo cleanup sequences.
